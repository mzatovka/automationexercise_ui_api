...
# фисктура, которая создаёт пользователя через api-запрос 
# эта фикстура создана вместо user, которая создаёт пользоавтеля через ui, что занимает много времени
# если будет использоваться фикстура user, каждый раз когда нужно будет создать пользователя, будет выполняться много лишних действий
@pytest.fixture
